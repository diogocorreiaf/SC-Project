...
char P[18][9] = {"243f6a88", "85a308d3", "13198a2e", "03707344", "a4093822",
...
void sub_key_gen(const char *key) {
    int j = 0;
    for (int i = 0; i < 18; i++) {
        // Get 8 characters from the key starting from index j
        char subkey[9];
        strncpy(subkey, key + j, 8);
        subkey[8] = '\0'; // Null-terminate the string
        
        // XOR the subkey with P[i]
        char *result = xor(P[i], subkey);
        strncpy(P[i], result, 8);
...
void blowfish_encrypt_file(FILE *input_file, const char *output_file) {
    ...
    // Force the key to be "aabb09182736ccdd"
    const char forced_key[] = "aabb09182736ccdd";

    // Generate subkeys
    sub_key_gen( forced_key);
