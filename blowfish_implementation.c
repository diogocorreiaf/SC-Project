...
// Helper function to convert a 64-bit block to two 32-bit halves
void block_to_halves(const char *block, uint32_t *left, uint32_t *right) {
    *left = *((uint32_t *)block);
    *right = *((uint32_t *)(block + 4));
}

// Helper function to convert two 32-bit halves to a 64-bit block
void halves_to_block(uint32_t left, uint32_t right, char *block) {
    memcpy(block, &left, sizeof(uint32_t));
    memcpy(block + 4, &right, sizeof(uint32_t));
}

void feistel_round(uint32_t *left, uint32_t *right, uint32_t round_key) {
    // Step 1: Save the original value of the right half
    uint32_t original_right = *right;

    // Step 2: Perform the Feistel function
    // (a) Apply the S-boxes
    *right = *right ^ S[0][(*left >> 24) & 0xFF];
    *right += S[1][(*left >> 16) & 0xFF];
    *right ^= S[2][(*left >> 8) & 0xFF];
    *right += S[3][*left & 0xFF];
    // (b) Apply the XOR with the round key
    *right ^= round_key;

    // Step 3: Swap the left and right halves
    *left ^= *right;
    *right ^= *left;
    *left ^= *right;

    // Step 4: Restore the original value of the right half
    *left = original_right;
}
...
char *encrypt(const char *plaintext) {
    ...
    char *ciphertext = (char *)malloc(num_blocks * 8 + 1);
    if (ciphertext == NULL) {
        perror("Memory allocation failed");
        return NULL;
    }
    ciphertext[num_blocks * 8] = '\0'; // Null-terminate the string

    // Process each block
    for (size_t i = 0; i < num_blocks; i++) {
        uint32_t left, right;
        // Convert the block to halves
        block_to_halves(plaintext + i * 8, &left, &right);

        // Encrypt the block using each subkey
        for (int round = 0; round < 16; round++) {
            feistel_round(&left, &right, round);
        }
        ...
        // Convert halves back to block and append to ciphertext
        halves_to_block(left, right, ciphertext + i * 8);
    }
