...
void encrypt_file(FILE *plaintext_file, FILE *ciphertext_file, size_t plaintext_size) {
    ...
    while (bytes_left > 0) {
        ...
        if (bytes_read < BLOCK_SIZE) {
            ...
            memset(plaintext_buffer + bytes_read, paddingSize, BLOCK_SIZE - bytes_read);
        }
    ...
    while (bytes_left > 0) {
        ...
        *((uint32_t *)(decrypted_buffer + sizeof(uint32_t))) = xr;
        ...
        if (bytes_left == bytes_read) {
            ...
            fwrite(decrypted_buffer, 1, BLOCK_SIZE - paddingSize, decrypted_file);
        }
        else {
            fwrite(decrypted_buffer, 1, BLOCK_SIZE, decrypted_file);
        }
